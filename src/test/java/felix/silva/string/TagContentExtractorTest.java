...
class TagContentExtractorTest {
    private final TagContentExtractor solution = new TagContentExtractor();

    @Test
    void testSolution() {
        var answer = solution.extract("<h1>Nayeem loves counseling</h1>");
        assertEquals("Nayeem loves counseling", answer);
        answer = solution.extract("<h1>had<h1>public</h1></h1>");
        assertEquals("public", answer);
        answer = solution.extract("<h1>had<h1>public</h1515></h1>");
        assertEquals("None", answer);
    }

    @Test
    void testSolution1() {
        var answer = solution.extract("<>hello</>");
        assertEquals("None", answer);
    }

    @Test
    void testSolution2() {
        var answer = solution.extract("<>hello</><h>dim</h>");
        assertEquals("dim", answer);
    }

    @Test
    void testSolution3() {
        var answer = solution.extract("<h1>First</h1><h2>Second</h2>");
        assertEquals("First\nSecond", answer);
    }

    @Test
    void testSolution4() {
        var answer = solution.extract("qqoNVOmJDG@6IBDZoEmk9337LswEL&TQnLCuR`04XD%1t{G)Jmi_iNEXKwp&<iBKMbDGtF4v@coLsF1_LqgTJ3cSp& 3a~I&Q(j0h_w~Vk(oBZCL#vhYY9%c><wkjkTvAuA8Zk}n_l7Si\"-pfm`M8YE3F}4`YQyChgC3uRbyTvu>bMUGux)5n7L=" +
                "{M}e^`0xlSm5ce}ehiE}CJ6y0KPd~~B~ak5$PTdPGv}QnXpw6n9V8wVCVaTRTgLKkeF</wkjkTvAuA8Zk}n_l7Si\"-pfm`M8YE3F}4`YQyChgC3uRbyTvu>haZQKlWPxlRqXXkKHo=FDofc6$_S-GWA&m0zT*D~uorf_nAF^ym*U&6hGAI)s<XshvNhnnNbeVDuxRcQAgTpWZ-kqIps-" +
                "@@}Uwq0J3Z06Y5mZgB9><FbTSC#F104{py9Xl6s{yi-R~}k5Fv4i1kCgmBY7P=vVj-j48xUg8x9BCxl~Y><lyxRRMqnMBGj1_d7Qqh5Ebn7 aMb{Q0Dm){9~I0DTS8BZ7+bui~)rQ\"2Yb4f>EeZWvJvHIk</XshvNhnnNbeVDuxRcQAgTpWZ-kqIps-@@}Uwq0J3Z06Y5mZgB9><BkkZV631Pnj}#%TWhZn@Y><kXjDpTvLA^tnXYb`h+cA J2\n" +
